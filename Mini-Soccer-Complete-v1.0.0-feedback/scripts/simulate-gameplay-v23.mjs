...
const passBuckets={};
const bucket=(group,key,ok,error)=>{const id=`${group}:${key}`,entry=passBuckets[id]??={total:0,arrived:0,maxError:0};entry.total++;if(ok)entry.arrived++;entry.maxError=Math.max(entry.maxError,error);passBuckets[id]=entry};
...
for(const format of [3,4]){
  ...
  for(let i=0;i<800;i++){
    const angle=(i%24)/24*Math.PI*2,distance=85+(i%32)*15.5,charge=.15+(i%70)/100,passer=mkPlayer(660,420,0,78+(i%20),i%3===0?"MED":"DEL",(rng()-.5)*120,(rng()-.5)*120),receiver=mkPlayer(660+Math.cos(angle)*distance,420+Math.sin(angle)*distance,0,76+(i%22),i%2?"DEL":"MED",(rng()-.5)*150,(rng()-.5)*150),opponent=mkPlayer(900,700,1,86,"DEF");
    const plan=passSystem.plan({origin:{x:passer.x,y:passer.y},passer,players:[passer,receiver,opponent],teamStart:0,teamEnd:2,selectedReceiver:1,receiverLocked:true,confidence:1,userIntentDirection:{x:Math.cos(angle),y:Math.sin(angle)},charge,assist:"ASSISTED",format,fieldDiagonal:diagonal,bounds,pressure:(i%9)/12,receiverPressure:120+(i%5)*35,through:i%7===0,rng:()=>.5});
    passScenarios++;
    ...
    const arrived=closest<34;maxPassError=Math.max(maxPassError,closest);if(arrived)passArrivals++;
    bucket("format",`${format}v${format}`,arrived,closest);
    bucket("type",plan.passType,arrived,closest);
    bucket("charge",charge<.30?"0.15-0.29":charge<.50?"0.30-0.49":charge<.70?"0.50-0.69":"0.70-0.84",arrived,closest);
    bucket("distance",distance<200?"85-199":distance<350?"200-349":"350-566",arrived,closest);
    bucket("combo",`${plan.passType}|${distance<200?"short":distance<350?"mid":"long"}|${charge<.30?"low":charge<.50?"mid":"high"}`,arrived,closest);
  }
}
const diagnostic=Object.fromEntries(Object.entries(passBuckets).map(([key,value])=>[key,{...value,rate:Number((value.arrived/value.total*100).toFixed(2)),maxError:Number(value.maxError.toFixed(2))}]));
console.log("V2.3 PASS DIAGNOSTICS",JSON.stringify(diagnostic,null,2));
