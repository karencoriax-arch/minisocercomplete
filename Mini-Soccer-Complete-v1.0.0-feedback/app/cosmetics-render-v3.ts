...
export function hudClassV3(equipped:EquippedCosmeticsV3){return `cosmetic-${safe(equipped.HUD??"hud_classic")}`}
export function goalFxClassV3(equipped:EquippedCosmeticsV3){const id=equipped.GOAL_FX??"";return id?`goal-fx-${safe(id.replace(/^goal_/,""))}`:""}
export function celebrationClassV3(equipped:EquippedCosmeticsV3){const id=equipped.CELEBRATION??"";return id?`celebration-${safe(id.replace(/^celebration_/,""))}`:""}
export function stadiumThemeClassV3(equipped:EquippedCosmeticsV3){const id=equipped.STADIUM_THEME??"";return id?`stadium-theme-${safe(id.replace(/^stadium_/,""))}`:""}
export function bannerClassV3(equipped:EquippedCosmeticsV3){const id=equipped.BANNER??"";return id?`banner-${safe(id.replace(/^banner_/,""))}`:""}
